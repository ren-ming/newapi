...
reset-setup:
	...
	@if docker compose -f $(DEV_COMPOSE_FILE) ps --services --status running | grep -qx "$(DEV_DB_SERVICE)"; then \
		...
		docker compose -f $(DEV_COMPOSE_FILE) exec -T $(DEV_DB_SERVICE) \
			...
			-e 'DELETE FROM setups;' \
			-e 'DELETE FROM users WHERE role = 100;' \
			-e "DELETE FROM options WHERE key IN ('SelfUseModeEnabled', 'DemoSiteEnabled');"; \
